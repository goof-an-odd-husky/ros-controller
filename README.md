...
Use if you want a ros2 environment, with everything setup (including the simulation)
...
- Controller node - controls Husky to move to coordinates set by visualizer
    ...
    - Preferably should be run on the machine itself (or at least over something faster than Wi-Fi)
- Visualizer node - shows the obstacles, the global path and local trajectory, created by controller
    ...
    - Can be run on a separate machine on the network
